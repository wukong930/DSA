...
import { Activity, BarChart3, BriefcaseBusiness, Clock, FileText, Home, LogOut, MessageSquareQuote, Settings2, Star, TrendingUp } from 'lucide-react';
...
const NAV_ITEMS: NavItem[] = [
  ...
  { key: 'strategy-bt', label: '策略回测', to: '/strategy-backtest', icon: TrendingUp },
      ...
      <nav className="flex flex-1 flex-col gap-1.5" aria-label="主导航">
        {NAV_ITEMS.map(({ key, label, to, icon: Icon, exact, badge }) => (
          <NavLink
            ...
            className={({ isActive }) =>
              cn(
                'group relative flex items-center gap-3 overflow-hidden rounded-xl text-sm transition-all',
          ...
          >
            {({ isActive }) => (
              <>
                <Icon className={cn('ml-1 h-5 w-5 shrink-0', isActive ? 'text-[hsl(var(--primary))]' : 'text-current')} />
