...
class Config:
    ...
    agent_llm_call_timeout_s: int = 60  # Per-call LLM timeout (seconds); prevents a single stuck API call from blocking the pipeline
    ...
    email_sender_name: str = "DSA股票分析助手"  # 发件人显示名称
    ...
    # 推送过滤：仅推送有操作信号的结果（买入/卖出/减仓），观望/持有不推送
    notify_actionable_only: bool = False
    ...
    def _load_from_env(cls) -> 'Config':
        ...
        return cls(
            ...
            agent_llm_call_timeout_s=parse_env_int(
                os.getenv('AGENT_LLM_CALL_TIMEOUT_S'),
                60,
                field_name='AGENT_LLM_CALL_TIMEOUT_S',
                minimum=10,
            ),
            ...
            email_sender_name=os.getenv('EMAIL_SENDER_NAME', 'DSA股票分析助手'),
            ...
            notify_actionable_only=os.getenv('NOTIFY_ACTIONABLE_ONLY', 'false').lower() == 'true',
